...
export function update_vals(vals) {
  ...
  d3.selectAll("boxes").each(function(mbar){
    let max = mbar.max?lookup(mbar.max,vals):Math.max.apply(null, mbar.values.map(v=>lookup(v,vals)))
    var last = 0
    d3.select(this)
    .selectAll("box").each(function(box){
      let small_percent = (100*lookup(box.val,vals)/max)||0;
      d3.select(this)
      .style("--last-percent",last)
      .style("--fill-percent",small_percent)
      last = small_percent
    })
  })
...
export function render_values(group,values) {
  ...
  d3.select("#"+group).selectAll("value").data(values,key_by_id)
    .join(enter=>enter.append("value")
      .call(value=>value.append("main-label").text(d=>d.name))
      .call(value=>value.append("val").datum(d=>d.value))
      .each(function(d) {if (d.diff != null){
        let val = d3.select(this)
        val.append("span").text(" + ")
        val.append("val").datum(d.diff)
        val.append("span").text("/s")
      }})
      .call(value=>value.append("desc").text(d=>d.desc))
    )
}
...
export function render_bars(group,bars) {
  ...
  .join(enter=>enter.append("multibar")
    .call(mbar=>mbar.append("main-label")
      ...
      .join(enter=>enter.append("box")
        .call(box=>box.append("main-label")
          .text(d=>d.name)
        ).call(box=>box.append("val").datum(d=>d.val))
        .each(function(d) {d3.select(this).classed(d.name,true)})
      )
